...
def run_video_combinator(params: dict):
    """
    视频组合任务入口，可被 Worker 调用。
    支持所有配置参数。
    """
    job = get_current_job()
    lock_key = "lock:video_combinator"
    ...
    try:
        ...
        redis_conn = Redis(
            ...
            port=6379,
            db=0,
            # username='default',  # Redis 6+ 如果不设置 ACL 用户可以注释掉
            password='rkwork',
    ...
    try:
        ...
        # main() 或其他处理函数
        # 从 data 获取所有参数
        input_folder = params["data"].get("input_folder", "catoutput/baby_cat/")
        output_folder = params["data"].get("output_folder", "./rkwork/output_videos")
        min_duration = params["data"].get("min_duration", 20)
        max_duration = params["data"].get("max_duration", 45)
        max_clips_per_video = params["data"].get("max_clips_per_video", 15)
        max_videos = params["data"].get("max_videos", 5)
        max_usage = params["data"].get("max_usage", 15)
        ttf_path = params["data"].get("ttf_path", "fonts/汉仪晓波花月圆W.ttf")
